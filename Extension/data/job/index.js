...
document.getElementById('hrefs').onsubmit = async e => {
  ...
  try {
    ...
    if (!file) {
      // Try to start from saved directory (from Settings)
      const savedDir = window.getRootDirectory?.();
      if (savedDir) {
        opts.startIn = savedDir;
      }
